...
pub fn secret_agent_stream(
    ...
    // The new `stream::channel` signature takes `AsyncFnOnce(Sender<T>)`,
    // which the compiler can't infer `T` for from the body alone — pin it
    // via the closure parameter type.
    iced_futures::stream::channel::<Event>(4, move |mut msg_tx: futures::channel::mpsc::Sender<Event>| async move {
