...
// Note: upstream imported `iced_futures::{Subscription, stream}` here for
// the `subscription()` wrapper that was removed (see active_conns.rs).
// The crate still depends on `iced_futures` indirectly through callers
// that build their own `Subscription`s; we keep the dep declaration.
...
// Upstream top-level `subscription` wrapper removed; see active_conns.rs.
// Cosmonaut's wifi page constructs its own `Subscription` around `watch`.
